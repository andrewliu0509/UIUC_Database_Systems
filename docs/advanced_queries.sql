...
FROM 
    User JOIN Favorite_Report
        ON User.user_id = Favorite_Report.favorite_user_id
    JOIN User_Reporting
        ON Favorite_Report.reporting_id = User_Reporting.reporting_id
        AND Favorite_Report.reporting_user_id = User_Reporting.user_id
...
FROM (
    ...
    WHERE h2.median_sale_price < 700,000 AND h2.property_type = 'Townhouse'
    )
UNION
(
    SELECT l3.city, h3.period_begin, h3.period_end
    FROM [Location] l3 NATURAL JOIN House h3
    WHERE h3.median_sale_price < 1,000,000 AND h3.property_type = 'Residential'
